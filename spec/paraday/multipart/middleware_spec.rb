...
RSpec.describe Paraday::Multipart::Middleware do
  let(:options) { {} }
  let(:conn) do
    Paraday.new do |b|
      ...
      b.adapter :test do |stub|
        stub.post('/echo') do |env|
          ...
          expect(env[:body]).to be_a_kind_of(Paraday::Multipart::CompositeReadIO)
    ...
    let(:payload) do
      {
        a: 1,
        b: {
          c: Paraday::Multipart::FilePart.new(
            __FILE__,
            'text/x-ruby',
            nil,
  ...
  context 'FilePart: when providing json and IO content in the same payload' do
    ...
    let(:payload) do
      {
        json: Paraday::Multipart::ParamPart.new(json, 'application/json'),
        io: Paraday::Multipart::FilePart.new(io, 'application/pdf')
      }
    end
    ...
    let(:payload) do
      {
        a: 1,
        b: [
          Paraday::Multipart::FilePart.new(io, 'application/pdf'),
          Paraday::Multipart::FilePart.new(io, 'application/pdf')
        ]
      }
    end
