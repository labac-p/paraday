...
RSpec.describe Paraday::MiddlewareRegistry do
  before do
    stub_const('CustomMiddleware', custom_middleware_klass)
  end
  let(:custom_middleware_klass) { Class.new(Paraday::Middleware) }
  let(:dummy) { Class.new { extend Paraday::MiddlewareRegistry } }
