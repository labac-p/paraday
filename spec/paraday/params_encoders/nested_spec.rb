...
RSpec.describe Paraday::NestedParamsEncoder do
  ...
  it 'encodes rack compat' do
    ...
    result   = Paraday::Utils.unescape(Paraday::NestedParamsEncoder.encode(params)).split('&')
  ...
  it 'encodes empty string array value' do
    ...
    result = Paraday::NestedParamsEncoder.encode(foo: { bar: '' }, baz: '')
    expect(result).to eq(expected)
  end

  it 'encodes nil array value' do
    expected = 'baz&foo%5Bbar%5D'
    result = Paraday::NestedParamsEncoder.encode(foo: { bar: nil }, baz: nil)
    expect(result).to eq(expected)
  end
