...
shared_examples 'default connection options' do
  after { Paraday.default_connection_options = nil }

  it 'works with implicit url' do
    conn = Paraday.new 'http://httpbingo.org/foo'
    expect(conn.options.timeout).to eq(10)
  end

  it 'works with option url' do
    conn = Paraday.new url: 'http://httpbingo.org/foo'
    expect(conn.options.timeout).to eq(10)
  end

  it 'works with instance connection options' do
    conn = Paraday.new 'http://httpbingo.org/foo', request: { open_timeout: 1 }
    expect(conn.options.timeout).to eq(10)
    expect(conn.options.open_timeout).to eq(1)
  end

  it 'default connection options persist with an instance overriding' do
    conn = Paraday.new 'http://nigiri.com/bar'
    conn.options.timeout = 1
    expect(Paraday.default_connection_options.request.timeout).to eq(10)

    other = Paraday.new url: 'https://httpbingo.org/foo'
  ...
  it 'default connection uses default connection options' do
    expect(Paraday.default_connection.options.timeout).to eq(10)
  end
end

RSpec.describe Paraday::Connection do
  let(:conn) { Paraday::Connection.new(url, options) }
  let(:url) { nil }
  let(:options) { nil }
  ...
  describe '.new' do
    ...
    context 'with implicit url param' do
      # Paraday::Connection.new('http://httpbingo.org')
      let(:url) { address }
    ...
    context 'with explicit url param' do
      # Paraday::Connection.new(url: 'http://httpbingo.org')
      let(:url) { { url: address } }
    ...
    context 'with custom builder' do
      let(:custom_builder) { Paraday::RackBuilder.new }
    ...
    context 'with custom headers' do
      let(:options) { { headers: { user_agent: 'Paraday' } } }

      it { expect(subject.headers['User-agent']).to eq('Paraday') }
    end
    ...
    context 'with empty block' do
      let(:conn) { Paraday::Connection.new {} }
    ...
    context 'with block' do
      let(:conn) do
        Paraday::Connection.new(params: { 'a' => '1' }) do |paraday|
          paraday.adapter :test
          paraday.url_prefix = 'http://httpbingo.org/omnom'
        end
      end
  ...
  describe '#close' do
    before { Paraday.default_adapter = :test }
    after { Paraday.default_adapter = nil }
  ...
  describe '#build_exclusive_url' do
    ...
    it 'allows to provide params argument' do
      conn.url_prefix = 'http://httpbingo.org/nigiri'
      conn.params = { a: 1 }
      params = Paraday::Utils::ParamsHash.new
      params[:a] = 2
  ...
  describe '#to_env' do
    ...
    it 'without braketizing repeated params in query' do
      @params = { 'a' => [1, 2] }
      conn.options.params_encoder = Paraday::FlatParamsEncoder
  ...
  describe 'proxy support' do
    ...
    it 'allows when url in no proxy list' do
      with_env 'http_proxy' => 'http://proxy.com', 'no_proxy' => 'example.com' do
        conn = Paraday::Connection.new('http://example.com')
        expect(conn.proxy).to be_nil
      end
    end
    ...
    it 'allows when prefixed url is not in no proxy list' do
      with_env 'http_proxy' => 'http://proxy.com', 'no_proxy' => 'example.com' do
        conn = Paraday::Connection.new('http://prefixedexample.com')
        expect(conn.proxy.host).to eq('proxy.com')
      end
    end
    ...
    it 'allows when subdomain url is in no proxy list' do
      with_env 'http_proxy' => 'http://proxy.com', 'no_proxy' => 'example.com' do
        conn = Paraday::Connection.new('http://subdomain.example.com')
        expect(conn.proxy).to be_nil
      end
    end
    ...
    it 'allows when ip address is not in no proxy list but url is' do
      with_env 'http_proxy' => 'http://proxy.com', 'no_proxy' => 'localhost' do
        conn = Paraday::Connection.new('http://127.0.0.1')
        expect(conn.proxy).to be_nil
      end
    end
    ...
    it 'allows when url is not in no proxy list but ip address is' do
      with_env 'http_proxy' => 'http://proxy.com', 'no_proxy' => '127.0.0.1' do
        conn = Paraday::Connection.new('http://localhost')
        expect(conn.proxy).to be_nil
      end
    end
    ...
    it 'allows in multi element no proxy list' do
      with_env 'http_proxy' => 'http://proxy.com', 'no_proxy' => 'example0.com,example.com,example1.com' do
        expect(Paraday::Connection.new('http://example0.com').proxy).to be_nil
        expect(Paraday::Connection.new('http://example.com').proxy).to be_nil
        expect(Paraday::Connection.new('http://example1.com').proxy).to be_nil
        expect(Paraday::Connection.new('http://example2.com').proxy.host).to eq('proxy.com')
      end
    end
  ...
  describe '#respond_to?' do
    it { expect(Paraday.respond_to?(:get)).to be_truthy }
    it { expect(Paraday.respond_to?(:post)).to be_truthy }
  end

  describe 'default_connection_options' do
    context 'assigning a default value' do
      before do
        Paraday.default_connection_options = nil
        Paraday.default_connection_options.request.timeout = 10
      end
    ...
    context 'assigning a hash' do
      before { Paraday.default_connection_options = { request: { timeout: 10 } } }
    ...
    context 'preserving a user_agent assigned via default_conncetion_options' do
      around do |example|
        old = Paraday.default_connection_options
        Paraday.default_connection_options = { headers: { user_agent: 'My Agent 1.2' } }
        example.run
        Paraday.default_connection_options = old
      end

      context 'when url is a Hash' do
        let(:conn) { Paraday.new(url: 'http://example.co', headers: { 'CustomHeader' => 'CustomValue' }) }

        it { expect(conn.headers).to eq('CustomHeader' => 'CustomValue', 'User-Agent' => 'My Agent 1.2') }
      end

      context 'when url is a String' do
        let(:conn) { Paraday.new('http://example.co', headers: { 'CustomHeader' => 'CustomValue' }) }
  ...
  describe 'request params' do
    ...
    context 'with flat params encoder' do
      let(:options) { { request: { params_encoder: Paraday::FlatParamsEncoder } } }
      let(:stubs) { Paraday::Adapter::Test::Stubs.new }
