...
RSpec.describe Paraday::Error do
  describe '.initialize' do
    ...
    context 'with exception only' do
      ...
      it { expect(subject.inspect).to eq('#<Paraday::Error wrapped=#<RuntimeError: test>>') }
    ...
    context 'with response hash' do
      ...
      it { expect(subject.inspect).to eq('#<Paraday::Error response={:status=>400}>') }
    ...
    context 'with string' do
      ...
      it { expect(subject.inspect).to eq('#<Paraday::Error #<Paraday::Error: custom message>>') }
      it { expect(subject.response_status).to be_nil }
      it { expect(subject.response_headers).to be_nil }
      it { expect(subject.response_body).to be_nil }
    end
    ...
    context 'with anything else #to_s' do
      ...
      it { expect(subject.inspect).to eq('#<Paraday::Error #<Paraday::Error: ["error1", "error2"]>>') }
      it { expect(subject.response_status).to be_nil }
      it { expect(subject.response_headers).to be_nil }
      it { expect(subject.response_body).to be_nil }
    end
    ...
    context 'with exception and response object' do
      ...
      let(:response) { Paraday::Response.new(status: 400, response_headers: headers, response_body: body) }
