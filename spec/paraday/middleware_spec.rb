...
RSpec.describe Paraday::Middleware do
  ...
  describe '#on_error' do
    ...
    it 'is called by #call' do
      expect(app).to receive(:call).and_raise(Paraday::ConnectionFailed)
      is_expected.to receive(:call).and_call_original
      is_expected.to receive(:on_error)

      expect { subject.call(double) }.to raise_error(Paraday::ConnectionFailed)
    end
  end
