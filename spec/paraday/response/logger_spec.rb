...
RSpec.describe Paraday::Response::Logger do
  ...
  let(:conn) do
    ...
    Paraday.new do |b|
      ...
      b.adapter :test do |stubs|
        ...
        stubs.get('/connection_failed') { raise Paraday::ConnectionFailed, 'Failed to open TCP connection' }
      end
    end
  end
  ...
  context 'with default formatter' do
    let(:formatter) { instance_double(Paraday::Logging::Formatter, request: true, response: true, filter: []) }

    before { allow(Paraday::Logging::Formatter).to receive(:new).and_return(formatter) }

    it 'delegates logging to the formatter' do
      expect(formatter).to receive(:request).with(an_instance_of(Paraday::Env))
      expect(formatter).to receive(:response).with(an_instance_of(Paraday::Env))
      conn.get '/hello'
    end
    ...
    context 'when no route' do
      it 'delegates logging to the formatter' do
        expect(formatter).to receive(:request).with(an_instance_of(Paraday::Env))
        expect(formatter).to receive(:exception).with(an_instance_of(Paraday::Adapter::Test::Stubs::NotFound))

        expect { conn.get '/noroute' }.to raise_error(Paraday::Adapter::Test::Stubs::NotFound)
      end
    end
  end
  ...
  context 'with custom formatter' do
    let(:formatter_class) do
      Class.new(Paraday::Logging::Formatter) do
        def request(_env)
  ...
  context 'when logging headers and errors' do
    ...
    it 'logs error message' do
      expect { conn.get '/connection_failed' }.to raise_error(Paraday::ConnectionFailed)
