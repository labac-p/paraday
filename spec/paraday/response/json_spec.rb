...
RSpec.describe Paraday::Response::Json, type: :response do
  let(:options) { {} }
  let(:headers) { {} }
  let(:middleware) do
    described_class.new(lambda { |env|
      Paraday::Response.new(env)
    }, **options)
  end
  ...
  def process(body, content_type = 'application/json', options = {})
    env = {
      ...
      request_headers: Paraday::Utils::Headers.new,
      response_headers: Paraday::Utils::Headers.new(headers)
    }
    env[:response_headers]['content-type'] = content_type if content_type
    yield(env) if block_given?
    middleware.call(Paraday::Env.from(env))
  end
  ...
  it 'chokes on invalid json' do
    expect { process('{!') }.to raise_error(Paraday::ParsingError)
  end

  it 'includes the response on the ParsingError instance' do
    process('{') { |env| env[:response] = Paraday::Response.new }
    raise 'Parsing should have failed.'
  rescue Paraday::ParsingError => e
    expect(e.response).to be_a(Paraday::Response)
  end
