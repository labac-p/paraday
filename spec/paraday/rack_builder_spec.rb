...
RSpec.describe Paraday::RackBuilder do
  ...
  context 'with default stack' do
    ...
    it { expect(subject[0]).to eq(Paraday::Request.lookup_middleware(:url_encoded)) }
    it { expect(subject.adapter).to eq(Paraday::Adapter.lookup_middleware(Paraday.default_adapter)) }
  end
  ...
  context 'with custom adapter only' do
    let(:conn) do
      Paraday::Connection.new do |builder|
    ...
    it { expect(subject[0]).to be_nil }
    it { expect(subject.adapter).to eq(Paraday::Adapter.lookup_middleware(:test)) }
  end
  ...
  context 'with custom handler and adapter' do
    ...
    it 'locks the stack after making a request' do
      ...
      expect { subject.use(Orange) }.to raise_error(Paraday::RackBuilder::StackLocked)
    end
    ...
    it 'allows to compare handlers' do
      expect(subject.handlers.first).to eq(Paraday::RackBuilder::Handler.new(Apple))
    end
  end
  ...
  context 'when having a single handler' do
    ...
    it 'raises an error trying to use an unregistered symbol' do
      expect { subject.use(:apple) }.to raise_error(Paraday::Error) do |err|
        expect(err.message).to eq(':apple is not registered on Paraday::Middleware')
      end
    end
  end
  ...
  context 'when adapter is added with named options' do
    after { Paraday.default_adapter_options = {} }
    let(:conn) { Paraday::Connection.new {} }

    let(:cat_adapter) do
      Class.new(Paraday::Adapter) do
        attr_accessor :name
    ...
    it 'adds a handler to construct adapter with named options' do
      Paraday.default_adapter = cat_adapter
      Paraday.default_adapter_options = { name: 'Chloe' }
  ...
  context 'when middleware is added with named arguments' do
    ...
    let(:dog_middleware) do
      Class.new(Paraday::Middleware) do
        attr_accessor :name
  ...
  context 'when a middleware is added with named arguments' do
    ...
    it 'adds a handler to construct request adapter with options passed to request' do
      Paraday::Request.register_middleware cat_request: cat_request
  ...
  context 'when a middleware is added with named arguments' do
    ...
    it 'adds a handler to construct response adapter with options passed to response' do
      Paraday::Response.register_middleware fish_response: fish_response
  ...
  context 'when a plain adapter is added with named arguments' do
    ...
    it 'adds a handler to construct adapter with options passed to adapter' do
      Paraday::Adapter.register_middleware rabbit_adapter: rabbit_adapter
