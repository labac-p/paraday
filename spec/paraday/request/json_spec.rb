...
RSpec.describe Paraday::Request::Json do
  let(:middleware) { described_class.new(->(env) { Paraday::Response.new(env) }) }

  def process(body, content_type = nil)
    env = { body: body, request_headers: Paraday::Utils::Headers.new }
    env[:request_headers]['content-type'] = content_type if content_type
    middleware.call(Paraday::Env.from(env)).env
  end
  ...
  context 'with encoder' do
    ...
    context 'when encoder is passed as object' do
      let(:middleware) { described_class.new(->(env) { Paraday::Response.new(env) }, { encoder: encoder }) }
    ...
    context 'when encoder is passed as an object-method pair' do
      let(:middleware) { described_class.new(->(env) { Paraday::Response.new(env) }, { encoder: [encoder, :dump] }) }
