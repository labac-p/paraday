...
RSpec.describe Paraday::Request::Instrumentation do
  ...
  let(:config) { {} }
  let(:options) { Paraday::Request::Instrumentation::Options.from config }
  ...
  it { expect(options.name).to eq('request.paraday') }
  ...
  it 'instruments with default name' do
    ...
    expect(name).to eq('request.paraday')
