...
RSpec.describe Paraday::Request::UrlEncoded do
  let(:conn) do
    Paraday.new do |b|
  ...
  it 'works with nested params' do
    ...
    expect(Paraday::Utils.parse_nested_query(response.body)).to eq(expected)
  end

  it 'works with non nested params' do
    response = conn.post('/echo', dimensions: %w[date location]) do |req|
      req.options.params_encoder = Paraday::FlatParamsEncoder
    end
    expect(response.headers['Content-Type']).to eq('application/x-www-form-urlencoded')
    expected = { 'dimensions' => %w[date location] }
    expect(Paraday::Utils.parse_query(response.body)).to eq(expected)
  ...
  context 'customising default_space_encoding' do
    around do |example|
      Paraday::Utils.default_space_encoding = '%20'
      example.run
      Paraday::Utils.default_space_encoding = nil
    end
