...
RSpec.describe Paraday::Request do
  let(:conn) do
    Paraday.new(url: 'http://httpbingo.org/api',
  ...
  context 'when setting a header on setup with []= syntax' do
    let(:block) { proc { |req| req['Server'] = 'Paraday' } }
    let(:headers) { subject.to_env(conn).request_headers }

    it { expect(subject.headers['Server']).to eq('Paraday') }
    it { expect(headers['mime-version']).to eq('1.0') }
    it { expect(headers['server']).to eq('Paraday') }
  end
