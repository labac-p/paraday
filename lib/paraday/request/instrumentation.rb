...
module Paraday
  class Request
    ...
    class Instrumentation < Paraday::Middleware
      # Options class used in Request::Instrumentation class.
      Options = Paraday::Options.new(:name, :instrumenter) do
        remove_method :name
        # @return [String]
        def name
          self[:name] ||= 'request.paraday'
        end
      ...
      #   for Paraday requests.
      #   ActiveSupport::Notifications
      #     .subscribe('request.paraday') do |name, starts, ends, _, env|
      #     url = env[:url]
      ...
      # @option options [String] :name ('request.paraday')
...
Paraday::Request.register_middleware(instrumentation: Paraday::Request::Instrumentation)
