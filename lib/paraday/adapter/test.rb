...
module Paraday
  class Adapter
    # @example
    #   test = Paraday::Connection.new do
    #     use Paraday::Adapter::Test do |stub|
    ...
    class Test < Paraday::Adapter
        ...
        def new_stub(request_method, path, headers = {}, body = nil, &block)
          normalized_path, host =
            if path.is_a?(Regexp)
              ...
              [
                Paraday::Utils.normalize_path(path),
                Paraday::Utils.URI(path).host
              ]
            end
      ...
      Stub = Struct.new(:host, :path, :query, :headers, :body, :strict_mode, :block) do
        ...
        def matches?(env)
          ...
          request_path = Paraday::Utils.normalize_path(env[:url].path)
        ...
        def headers_match?(request_headers)
          if strict_mode
            headers_with_user_agent = headers.dup.tap do |hs|
              ...
              #       Users would not want to set Paraday's User-Agent explicitly.
      ...
      def call(env)
        ...
        status, headers, body =
          if timeout
            ::Timeout.timeout(timeout, Paraday::TimeoutError) do
        ...
        #   See https://github.com/lostisland/paraday/issues/1444
...
Paraday::Adapter.register_middleware(test: Paraday::Adapter::Test)
